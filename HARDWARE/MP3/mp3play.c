...
//	f_mount(0, &fs);			                         /* 挂载文件系统到0区 */  
 	                ...
 	                res = f_open(&fsrc, "0:/欢迎光临.mp3",  FA_READ); /* 打开文件 */
				...
				//	VS_SPI_SpeedHigh();
					...
					{			
						...
						{				
							...
							while ( count < 4096)		  /* SD卡读取一个sector，一个sector为512字节 */	
							{ 						
				  ...
				  sound_C=0;
					...
					f_close(&fsrc);	 /* 关闭打开的文件 */
					 ...
					 res = f_open(&fsrc, "0:/欢迎光临111.mp3", FA_OPEN_EXISTING | FA_READ); /* 打开文件 */
 //					res = f_open( &fsrc, finfo.lfname, FA_OPEN_EXISTING | FA_READ );
					res = f_lseek(&fsrc, 0);
...
{	 
	...
	{
	//	VS_WR_Cmd(0x00,0x0804);// 软件复位,新模式
	VS_WR_Cmd(SPI_MODE,aaa);// 软件复位,新模式	    
